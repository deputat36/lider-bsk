...
paths = [MIGRATION, TEST, EDGE, CONTRACT, TRANSPORT, VIEW]
...
for forbidden in ['SUPABASE_SERVICE_ROLE_KEY"', 'verify_jwt: false']:
    if forbidden in edge:
        errors.append('edge: forbidden marker: ' + forbidden)
...
for forbidden in ['.from(', '.insert(', '.update(', '.delete(', '.upsert(', '.rpc(', 'service_role', 'sb_secret_']:
    if forbidden in transport:
        errors.append('transport: browser write/elevation forbidden: ' + forbidden)
...
for forbidden in ['.insert(', '.update(', '.delete(', '.upsert(', '.rpc(', 'service_role', 'sb_secret_']:
    if forbidden in view:
        errors.append('view: direct browser write/elevation forbidden: ' + forbidden)

if errors:
    print('\n'.join(errors), file=sys.stderr)
    sys.exit(1)
print('Catalog staging write path: atomic service-role RPC behind JWT Edge, exact-staging UI gate, rollback acceptance: PASS')
