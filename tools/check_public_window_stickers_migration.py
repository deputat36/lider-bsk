...
PAGE = ROOT / 'nakleyki-na-vitrinu-borisoglebsk.html'
...
def main() -> None:
    ...
    for marker in (
        '<title>Наклейки на витрину в Борисоглебске — оформление стекла, двери, акций | РА Лидер</title>',
        '<meta name="description" content="Наклейки на витрину, дверь и стекло',
        '<link rel="canonical" href="https://www.lider-bsk.ru/nakleyki-na-vitrinu-borisoglebsk.html">',
        '<body class="page-window-stickers-detail">',
    ...
    if parser.inline_styles:
        raise SystemExit('Window stickers page must not contain inline CSS')
    if re.search(
        r'<script(?![^>]*\bsrc=)(?![^>]*\btype=["\']application/ld\+json["\'])[^>]*>',
        text,
        flags=re.I,
    ):
        raise SystemExit('Window stickers page must not contain executable inline JavaScript')
    ...
    for marker in (
        ...
        'body.page-entrance-detail .hero:before,body.page-window-stickers-detail .hero:before{content:""',
        '.hero__grid{position:relative;z-index:1;grid-template-columns:var(--entry-grid-columns)',
        '.links a{border:1px solid var(--leader-line);border-radius:999px',
        '@media(max-width:560px){.top__in{justify-content:center;text-align:center}.hero__actions .btn{width:100%}',
    ):
        require(detail, marker, 'assets/public-entry-detail.css')

    require(
        form_script,
        "'nakleyki-na-vitrinu-borisoglebsk.html':{service:'Наклейки'",
    ...
    print('Window stickers external detail CSS contract is valid.')
