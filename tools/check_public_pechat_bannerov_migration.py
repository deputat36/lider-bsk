...
def main() -> None:
    ...
    for marker in (
        '<title>Печать баннеров в Борисоглебске — фасад, акция, открытие | РА Лидер</title>',
        '<meta name="description" content="Печать баннеров в Борисоглебске:',
        '<link rel="canonical" href="https://www.lider-bsk.ru/pechat-bannerov-borisoglebsk.html">',
    ...
    if parser.inline_styles:
        raise SystemExit('Inline CSS returned to pechat-bannerov-borisoglebsk.html')
    if parser.executable_inline_scripts:
        raise SystemExit('Executable inline JavaScript returned to pechat-bannerov-borisoglebsk.html')
    ...
    require(
        form_script,
        "'pechat-bannerov-borisoglebsk.html':{service:'Баннер'",
    ...
    print('Pechat bannerov external CSS and shared preset contract is valid.')
