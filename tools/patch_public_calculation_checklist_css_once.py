...
def main() -> None:
    ...
    if CSS_LINK in page:
        if page.count(CSS_LINK) != 1:
            raise SystemExit("Unexpected number of calculation checklist CSS links")
        if STYLE_RE.search(page):
            raise SystemExit("Page contains both external calculation checklist CSS and inline style")
        if not CSS.is_file():
            raise SystemExit("CSS link exists but assets/public-calculation-checklist.css is missing")
        print("Calculation checklist CSS migration already applied; no changes needed.")
        return

    matches = list(STYLE_RE.finditer(page))
    if len(matches) != 1:
        raise SystemExit(f"Expected exactly one inline style block, found {len(matches)}")
    ...
    if CSS.exists():
        ...
        if existing != css:
            raise SystemExit("assets/public-calculation-checklist.css already exists with different content")
    else:
    ...
    if updated.count(CSS_LINK) != 1:
        raise SystemExit("Failed to insert calculation checklist CSS link exactly once")
    ...
    print(f"Migrated calculation checklist CSS: {len(css)} characters extracted.")
