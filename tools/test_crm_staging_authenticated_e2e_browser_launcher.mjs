...
assert.equal(operatorPlan().browser_transport_bridge, 'same_origin_user_jwt_exact_staging_proxy_with_db_assertion');

const runnerSource = await readFile(new URL('./run_crm_staging_authenticated_e2e.mjs', import.meta.url), 'utf8');
assert.doesNotMatch(runnerSource, /navigator\.sendBeacon\('\/__crm_e2e_staging_rpc_proxy'/);
...
assert.match(runnerSource, /new XMLHttpRequest\(\)/);
assert.match(runnerSource, /staging_proxy_xhr_timeout/);
...
assert.doesNotMatch(runnerSource, /status:201,headers:\{'Content-Type':'application\/json'/);
