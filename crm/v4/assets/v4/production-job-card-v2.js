...
function layoutNeedsCheck(job, order) {
  ...
  if (layoutApproved(text)) return false;
  if (text.includes('не треб')) return false;
  return true;
}
function renderLayoutAlert(job, order) {
...
async function printJob(jobId) {
  try {
    ...
    const layoutWarning = layoutNeedsCheck(job, order) ? '<div class="layout-alert">Внимание: дизайн / макет не согласован. Перед производством проверьте правки и ссылку на файл.</div>' : '';
    const win = window.open('', '_blank', 'noopener,noreferrer');
    if (!win) throw new Error('Браузер заблокировал окно печати');
    win.document.open();
    win.document.write(`<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>Производственный лист</title><style>@page{size:A4;margin:12mm}body{font-family:Arial,sans-serif;color:#111827}h1{font-size:22px}table{width:100%;border-collapse:collapse}td,th{border:1px solid #cbd5e1;padding:7px;text-align:left}.notice{border:2px solid #86efac;background:#ecfdf5;color:#065f46;border-radius:10px;padding:10px;margin:12px 0;font-weight:700}.layout-alert{border:2px solid #fdba74;background:#fff7ed;color:#9a3412;border-radius:10px;padding:10px;margin:12px 0;font-weight:800}</style></head><body><p>РА «Лидер» · ${new Date().toLocaleString('ru-RU')}</p><h1>${esc(job.title || order?.project_name || 'Производственное задание')}</h1><div class="notice">Лист для производства. Не содержит имя, телефон и контакты клиента.</div>${layoutWarning}<p><b>Заказ:</b> №${esc(order?.order_number || String(job.order_id || '').slice(0, 8))}</p><p><b>Статус:</b> ${esc(job.production_status || '—')} · <b>Срок:</b> ${dateRu(job.deadline)}</p><p><b>Статус макета:</b> ${esc(layoutStatus(job, order))}</p><p><b>Файл / макет:</b> ${esc(job.file_url || order?.layout_link || 'не указан')}</p><p><b>ТЗ:</b> ${esc(job.technical_task || 'не заполнено')}</p><table><thead><tr><th>№</th><th>Позиция</th><th>Кол-во</th><th>Размер</th><th>Комментарий</th></tr></thead><tbody>${rows}</tbody></table></body></html>`);
    win.document.close();
    win.print();
