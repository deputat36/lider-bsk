...
`<div class="v4-order-modal"><div class="v4-order-modal-card"><div class="v4-order-modal-head"><div><p class="v4-kicker">Карточка заказа</p><h2>№${esc(order.order_number || shortId(order.id))} — ${esc(order.project_name || 'Заказ')}</h2><p>${esc(order.client_name || 'Клиент не указан')} · ${esc(order.client_phone || 'телефон не указан')} · создано ${dateRu(order.created_at)}</p></div><button type="button" data-order-card-close>Закрыть</button></div><div class="v4-order-modal-grid"><div><span>Статус</span><b title="${esc(statusModel.known ? `Registry: ${statusModel.key}` : statusModel.warning)}">${esc(statusModel.label)}</b></div><div><span>Оплата</span><b>${esc(order.payment_status || 'Не оплачено')}</b></div><div><span>Срок</span><b>${dateRu(order.deadline)}</b></div><div><span>Дизайн / макет</span><b>${esc(order.layout_status || '—')}</b></div><div><span>Производство</span><b>${esc(order.production_status || '—')}</b></div><div><span>Тип</span><b>${esc(orderType)}</b></div><div><span>Клиенту</span><b>${money(order.client_total)}</b></div><div><span>Себестоимость</span><b>${money(order.contractor_cost)}</b></div><div><span>Прибыль</span><b>${money(order.profit)}</b></div><div><span>Баланс</span><b>${money(order.balance)}</b></div></div>${statusWarning}<div class="v4-order-modal-actions">${order.lead_id ? `<button type="button" class="v4-primary" data-order-card-open-lead="${esc(order.lead_id)}">Открыть заявку</button>` : ''}<button type="button" data-order-card-close>Закрыть</button></div>${renderDesign(order)}${renderFinance(order, payments, expenses)}<section class="v4-order-modal-section"><h3>Позиции заказа</h3><div class="v4-order-modal-items">${renderItems(items)}</div></section></div></div>`;
}
