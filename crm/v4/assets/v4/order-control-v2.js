...
function unpaid(order) {
  return paymentNeedsAttention(order.payment_status);
}
...
function grouped() {
  ...
  return {
    ...
    unknownPayment: activeRows.filter((order) => !paymentStatusUiModel(order.payment_status).known),
...
function card(order, note = '') {
  ...
  const paymentModel = paymentStatusUiModel(order.payment_status);
  const statusWarning = statusModel.known ? '' : `<div class="v4-order-warnings" data-unknown-order-status="${esc(statusModel.raw)}">${esc(statusModel.warning)}</div>`;
  const paymentWarning = paymentModel.known ? '' : `<div class="v4-order-warnings" data-unknown-payment-status="${esc(paymentModel.raw)}">${esc(paymentModel.warning)}</div>`;
  return `<article class="v4-order-item ${cls}"><div class="v4-order-item-head"><h4>${esc(title(order))}</h4><small title="${esc(statusModel.known ? `Registry: ${statusModel.key}` : statusModel.warning)}">${esc(statusModel.label)}</small></div>${statusWarning}${paymentWarning}<small>${esc(order.client_name || 'Клиент не указан')} · ${esc(order.client_phone || 'телефон не указан')}</small><small>Срок: ${dateRu(order.deadline)} (${esc(deadline)})</small><small>Оплата: <span class="v4-order-payment-inline ${esc(paymentModel.cssClass)}" title="${esc(paymentModel.known ? `Registry: ${paymentModel.key}` : paymentModel.warning)}">${esc(paymentModel.label)}</span> · Сумма: ${money(order.client_total)}</small><small class="v4-order-design-inline ${designCheck ? 'is-warn' : ''}" data-order-control-design>Дизайн / макет: ${esc(layoutStatus(order))}</small>${note ? `<small>${esc(note)}</small>` : ''}<button type="button" data-open-order="${esc(order.id)}">Открыть заказ</button></article>`;
}
