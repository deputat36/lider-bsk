...
function renderPills(rows) {
  ...
  return rows
    .slice(0, 8)
    .map(([label, count]) => `<button type="button" class="lead-analytics-summary-pill" data-lead-analytics-search="${esc(label)}"><b>${esc(count)}</b> ${esc(label)}</button>`)
    .join('');
}
...
function ensureStyles() {
  ...
  style.id = STYLE_ID;
  style.textContent = `.lead-analytics-summary{margin:12px 0 14px;border:1px solid #dbeafe;background:#f8fafc;border-radius:16px;padding:12px;display:grid;gap:10px}.lead-analytics-summary h3{margin:0;color:#0f172a;font-size:15px}.lead-analytics-summary small{color:#64748b;font-weight:800}.lead-analytics-summary-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.lead-analytics-summary-box{border:1px solid #e2e8f0;background:#fff;border-radius:14px;padding:10px}.lead-analytics-summary-box strong{display:block;margin-bottom:8px;color:#1d4ed8}.lead-analytics-summary-pills{display:flex;flex-wrap:wrap;gap:6px}.lead-analytics-summary-pill{appearance:none;display:inline-flex;gap:5px;align-items:center;border-radius:999px;background:#eef2ff;border:1px solid #c7d2fe;color:#3730a3;padding:5px 8px;font-size:12px;font-weight:900;cursor:pointer}.lead-analytics-summary-pill:hover,.lead-analytics-summary-pill:focus{background:#dbeafe;border-color:#93c5fd;outline:none}.lead-analytics-summary-empty{color:#64748b;font-weight:800}@media(max-width:720px){.lead-analytics-summary-grid{grid-template-columns:1fr}}`;
...
function renderSummary() {
  ...
  container.innerHTML = `
    <div>
      ...
      <small>Derived-категории только для аналитики. Нажмите на категорию, чтобы применить поиск. Raw service/source в базе не меняются.</small>
    </div>
