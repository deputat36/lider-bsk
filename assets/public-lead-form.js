...
(function(){
      ...
      <form class="leader-lead-widget" data-leader-lead-widget>
        ...
        <div class="leader-lead-grid">
          <div class="leader-lead-span-6">
            ...
            <input id="${uid}-name" name="name" maxlength="200" placeholder="Например, Алексей">
          </div>
          ...
          <div class="leader-lead-span-12">
            <label for="${uid}-service">Что нужно?</label>
            <select id="${uid}-service" name="service">
              ...
              <option>Вывеска / наружная реклама</option>
              <option>Дизайн макета</option>
              <option>Соцсети и контент</option>
              <option>Яндекс Карты и 2ГИС</option>
              <option>Логотип / фирменный стиль</option>
          ...
          <div class="leader-lead-span-12">
            <label for="${uid}-message">Коротко опишите задачу</label>
            <textarea id="${uid}-message" name="message" maxlength="2000" rows="3" placeholder="Например: нужен баннер для магазина, примерно 2×1 м, макета нет"></textarea>
          </div>
        </div>
        ...
        <div class="leader-lead-details" data-leader-details hidden>
          <div class="leader-lead-grid">
            <div class="leader-lead-span-6">
              <label for="${uid}-city">Город</label>
              <input id="${uid}-city" name="city" maxlength="120" placeholder="Борисоглебск">
            </div>

            <div class="leader-lead-span-6">
              <label for="${uid}-contact">Как удобнее связаться?</label>
              <select id="${uid}-contact" name="contact_method">
                <option>Позвонить</option>
                <option>Написать ВКонтакте</option>
                <option>Написать в MAX</option>
            ...
            <div class="leader-lead-span-6">
              <label for="${uid}-quantity">Количество</label>
              <input id="${uid}-quantity" name="quantity" maxlength="120" placeholder="1 шт., 10 шт., 100 наклеек">
            </div>

            <div class="leader-lead-span-6">
              <label for="${uid}-deadline">Когда нужно?</label>
              <select id="${uid}-deadline" name="deadline">
                <option>Не срочно</option>
                <option>Как можно быстрее</option>
                <option>В течение 2–3 дней</option>
                <option>В течение недели</option>
                <option>К определённой дате</option>
                <option>Нужно обсудить</option>
              </select>
            </div>

            <div class="leader-lead-span-6">
              <label for="${uid}-mockup">Макет</label>
              <select id="${uid}-mockup" name="mockup">
                <option>Макета нет, нужен дизайн</option>
                <option>Макет готов</option>
                <option>Есть пример, нужно доработать</option>
                <option>Пока не знаю</option>
              </select>
            </div>

            <div class="leader-lead-span-6">
              <label for="${uid}-delivery">Доставка / монтаж</label>
              <select id="${uid}-delivery" name="delivery">
                <option>Не требуется</option>
                <option>Нужна доставка</option>
                <option>Нужен монтаж</option>
                <option>Доставка и монтаж</option>
                <option>Нужно обсудить</option>
              </select>
            </div>
          </div>
  ...
  async function submit(e){
    ...
    const contactMethod=field(form,'contact_method');
    ...
    if(contactMethod) parts.push('Связь: '+contactMethod);
