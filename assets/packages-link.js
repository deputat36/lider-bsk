(function(){
  ...
  function addUtilityCards(){
    ...
    if(grid){
      if(!document.getElementById('urgent-ads-card')){
        const urgent=document.createElement('article');
        urgent.className='card';
        urgent.id='urgent-ads-card';
        urgent.innerHTML='<div class="icon">⚡</div><h3>Срочная реклама</h3><p>Баннер, макет, афиша, пост или размещение в соцсетях, когда открытие или мероприятие уже скоро.</p><a href="srochnaya-reklama-borisoglebsk.html">Подробнее →</a>';
        grid.insertBefore(urgent,grid.children[1]||null);
      }
      if(!document.getElementById('social-ads-card')){
        ...
        grid.insertBefore(social,grid.children[2]||null);
      }
      if(!document.getElementById('cafe-ads-card')){
        ...
        grid.insertBefore(cafe,grid.children[3]||null);
      }
      if(!document.getElementById('events-ads-card')){
        ...
        grid.insertBefore(events,grid.children[4]||null);
      }
      if(!document.getElementById('store-ads-card')){
        ...
        grid.insertBefore(store,grid.children[5]||null);
      }
      if(!document.getElementById('packages-link-card')){
        ...
        grid.insertBefore(card,grid.children[6]||null);
      }
      if(!document.getElementById('calc-checklist-card')){
        ...
        grid.insertBefore(checklist,grid.children[7]||null);
      }
    }
  }
  function addHeroChecklistLink(){
    ...
    if(!quick)return;
    if(!document.getElementById('hero-urgent-ads-link')){
      const u=document.createElement('a');
      u.id='hero-urgent-ads-link';
      u.href='srochnaya-reklama-borisoglebsk.html';
      u.innerHTML='Срочно нужна реклама <span>→</span>';
      quick.appendChild(u);
    }
  ...
  function addTopNavUsefulLinks(){
    ...
    urgent.id='nav-urgent-link';
    urgent.href='srochnaya-reklama-borisoglebsk.html';
    ...
    nav.insertBefore(urgent,nav.querySelector('a[href="#request"]')||null);
  ...
  function addChecklistToContacts(){
    ...
    const u=document.createElement('a');
    u.id='contacts-urgent-link';
    u.className='btn btn--white';
    u.href='srochnaya-reklama-borisoglebsk.html';
    u.style.marginLeft='8px';
    u.textContent='Срочная реклама';
    const a=document.createElement('a');
    ...
    if(mainBtn){mainBtn.insertAdjacentElement('afterend',u);u.insertAdjacentElement('afterend',a);a.insertAdjacentElement('afterend',b);b.insertAdjacentElement('afterend',c)}
  }
  function run(){
    addUtilityCards();
