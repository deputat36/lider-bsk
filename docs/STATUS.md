...
- Frontend первого расчёта и новой версии разрешает бесплатную, убыточную работу и работу в ноль после явного подтверждения. Невалидные числа и количество ≤ 0 по-прежнему блокируются.
- Staging initial/version RPC согласованы: предупреждения сохраняются, RBAC, optimistic locking, receipts и исходные версии не меняются. [Staging browser E2E 36267024854](https://github.com/deputat36/lider-bsk/actions/runs/36267024854) прошёл: 4 версии (0, убыток, работа в ноль, прибыль), отмена без записи, replay/stale, КП → заказ → дизайн → производство → монтаж; manager/owner UI и API RBAC. Cleanup: Auth удалён, 21 категория residue = 0. Временный доступ ветки #526 к bootstrap удалён после проверки.
- Production Supabase не изменялся. Новая версия в production остаётся закрыта до rollout единого backend-контракта. Rollout/rollback: `CRM_SPECIAL_PRICE_ROLLOUT_2026-09-25.md`.

## Коммерческие направления и контекст заявки (#555)
...
- Исправлена перезапись выбранной услуги пустым или устаревшим query-параметром при отправке формы.
- Последняя UTM-кампания сохраняется на 30 минут в sessionStorage при переходах; новая кампания заменяет предыдущую целиком. Контактные данные в это хранилище не попадают.
- Проверка выполняет реальный submit с перехваченным HTTP и проверяет выбор, переходы, TTL, отказ хранилища и retry. Production-отправки не выполняются.
...
Исходные строгая и презентационная PDF-формы раскрывали comment позиции и internal_only строки. Теперь обе используют существующий publicOfferRows; запрос не выбирает comment, но получает data для client_title/visibility/components. Публичные условия расчёта и согласованный итог сохраняются. Новый regression проверяет реальные шаблоны и SELECT-проекцию; authenticated staging E2E расширен открытием обоих печатных окон после сохранения версии с внутренним комментарием. Authenticated staging run 34106225197 на c11a2590d5dc32e665fefeae43bf33b6b3994030 завершился успешно: обе формы реально открыты, внутренний комментарий сохранён в версии и отсутствует в печатном DOM; manager/owner RBAC и owner UI прошли. fatal_errors=[]; Auth user удалён, все 21 категории synthetic residue=0. Artifact 10012489397 SHA256 d7f21122cea6cba39221ec02d4dd1acfde727365de3bce5c23c0a3fbbfbd9ac3. Cloud Browser: авторизованная опубликованная карточка и текущий каталог доступны, 11 eager-модулей; production read-only baseline 69 catalog / 0 price logs, private prerequisites отсутствуют. Визуальная проверка PDF после публикации ещё требуется. Production migrations/Auth/backend routes не затрагиваются.
...
PR #519 продолжен от фактического head 5b464baded49d96d4f349d3f6e86d0546496ad22. Authenticated staging browser scenario расширен: неполная потребность, показ предупреждения, отмена, переход в редактор без сохранения, продолжение расчёта и проверка неизменности updated_at/missing_fields. Используется прежний E2E cleanup всех fixtures/Auth; run 34197836876 на b710728a58630dc7fa594bdcf209072dae489fea завершён успешно: readiness_cancel_edit_continue_without_mutation PASS, весь manager workflow и owner UI/RBAC PASS, fatal_errors=[]. Auth user удалён; все 21 категории synthetic residue=0. Artifact 10044619356 SHA256 e6516f79ed174c4a476ea219d2175020bf4888c8fd672b004496f2eeff756615. Production Supabase только read-only; новые backend routes не включаются.


## 2026-09-08 — #520: рабочий обзор заявки

В ветке карточка показывает этап, ответственного, ближайший контакт и задачу клиента перед основным действием. Потребность, расчёт, КП и заказ раскрываются отдельно; история и редкие действия свёрнуты. Существующие кнопки раскрывают целевой этап. Счётчики используют загруженное состояние без дополнительных запросов. Добавлены проверки существующих контейнеров и переходов. Authenticated staging E2E завершён на desktop (run 34228106829) и в узком Chrome 500 px (run 34228347788, head f8deecb20cf2c2b18f4f11aa32870cb89df58794): основной workflow, версии, pricing, print privacy, reload, RBAC и idempotency прошли; fatal_errors=[]. После каждого прогона Auth user удалён, все 21 счётчика residue=0. В узком окне summary имеет ширину 438 px и помещается по горизонтали. Это автоматическая проверка геометрии, не визуальная проверка mobile. На указанном head 57 checks success, 1 skipped. PR #521 merged 2026-09-11: 14b93bde413feb1d9a00754d6b00afa1c413438b. После публикации Cloud Browser: summary, раскрытие потребности/расчёта/КП/истории, reload и отсутствие app console errors подтверждены; production writes не выполнялись. Production backend routes и права не переключались.


## 2026-09-11 — #522: понятные подсказки менеджеру

В сводке заявок и проверке потребности технические термины заменены инструкциями сотруднику. Tooltip известного статуса показывает название этапа; предупреждение неизвестного статуса сохранено. Подсказка назначения ответственного теперь различает новую заявку и заявку на другом этапе. Маршруты и правила записи не меняются.


## 2026-09-14 — #524: ручная нулевая цена при добавлении

Cloud Browser подтвердил замену явно введённых 0 ₽ на 1300 ₽ при себестоимости 1000 ₽ в новой ручной позиции. Запись не сохранялась. Исправлен существующий builder: пустая цена включает автоматику, явный ноль сохраняется как ручная цена для своей позиции, услуги, подрядной позиции и букв/цифр. Старый запрет добавления бесплатных/убыточных позиций заменён проверкой корректности чисел; предупреждения об экономике сохранены. Regression на реальных функциях воспроизвёл дефект до исправления и проверяет последующее repricing. Проверка исправленного UI и CI ещё не завершены. Backend и production Supabase не меняются.


## Публичный сайт: вложенные 404 и служебные страницы (#535, 2026-09-15)

- Cloud Browser воспроизвёл отсутствие CSS и повторную 404 после перехода к услугам с вложенного адреса.
- Исправлены root-relative recovery href; добавлен noindex карточке Navigator без изменения callers/backend.
- PR #536 merged c538c0c6420ad0275d52618659ee9f8c498c8050. CI: 49 success, 1 skipped. Chromium run 34934057557 прошёл 1440/1024/768/390/360; скриншоты 1440/360 проверены. После Pages Cloud Browser подтвердил оформление вложенной 404 и переход к /uslugi.html.
- Supabase и формы не вызываются; synthetic данные не создаются.


## Главная: статическая навигация и доступное меню (#537, 2026-09-15)

- Убрана отложенная пересборка CSS/ссылок/текстов через 80/500/1200 ms. Постоянные коммерческие ссылки теперь в HTML, оформление в существующем brand layer.
- В header официальный master 250×66 / 225×60 / 184×49; пять основных пунктов. Старые разделы и форма v23 сохранены.
...
## 2026-09-20 — выбор формата вывески (#546)

- В рабочей ветке шесть общих задач заменены тремя форматами: отдельные буквы, вывеска на основе, комплект входной группы. Для каждого объяснено, с чего начать обсуждение.
- Добавлены четыре раскрывающихся вопроса (подсветка, замена, монтаж, срок) и прямые ссылки на баннеры/таблички/витрины. Неподтверждённых цен, сроков и изображений работ нет.
- Существующий browser regression дополнен keyboard/no-JS FAQ и переходом к странице баннеров. CI/визуальная проверка фиксируются в PR по результату, а не заранее.
- Supabase и отправка заявок не задействованы. Реальные фотографии остаются в отдельном intake #235.


## 2026-09-20 — баннеры и переиспользуемая подача услуг (#548)

- В рабочей ветке страница баннеров получила общую навигацию, три сценария вместо смешения изделий и этапов, подготовку расчёта, процесс, FAQ и связанные услуги. SEO и form v27 сохранены.
- Проверенный layout вывесок переиспользуется в существующем CSS через явный page-service-modern. Остальные девять simple-service страниц не подключены к нему. Общие классы названы service-*; новых JS модулей нет.
- Browser workflow теперь проверяет и вывески, и баннеры на пяти ширинах, включая разные service prefill. Окончательные CI/скриншоты/published smoke фиксируются в PR после выполнения.
- Supabase, Auth, Storage и отправка production заявок не задействованы; fixtures не создаются.


## 2026-09-20 — плёнка, наклейки и витрины (#550)

- Cloud Browser подтвердил предложение прикрепить фото при отсутствии file input на странице наклеек. В рабочей ветке подсказка исправлена: файлы передаются при обсуждении заказа.
- Три связанные страницы используют готовый opt-in layout и существующее меню: отдельные сценарии, данные для расчёта, процесс, FAQ, взаимные ссылки. Их service presets, версии form JS и JSON-LD сохранены.
- Существующий browser runner расширен до пяти страниц × пяти ширин, включая FAQ/no-JS, prefill, ссылки и запрет записи. Результаты CI и published smoke фиксируются в PR после выполнения.
- Supabase/Auth/Storage и production submission не вызываются. Реальные фотографии остаются отдельным blocker #235.
