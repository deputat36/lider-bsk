...
- `index.html` подключает расширенные расчёты, КП, связанный заказ, быстрый список заказов, карточку заказа и контроль заказов;
...
- `crm-v4-tabs-lite.js` знает вкладки `orders` и `order_control` как управляемые разделы.
...
- `orders-fast-loader-v1.js`, `order-card-v1.js` и `order-control-v2.js` существуют и подключены в `index.html`;
...
1. Проверить в браузере перенесённый контур CRM v4: вход, заявки, карточку заявки, потребности, расчёты, КП, создание заказа, связанный заказ, отдельный список заказов, карточку заказа, контроль заказов, контроль контактов, аудит заявок, диагностику.
2. Проверить аудит публичных заявок на реальной отправке формы.
3. Перенести финансовый контроль.
