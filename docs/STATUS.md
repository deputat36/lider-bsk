...
## Staging acceptance «заявка → заказ»

Обновление 2026-07-30:

- добавлены staging-only compatibility migration, catalog preflight и единый транзакционный сценарий заявки, потребности, двух версий расчёта, КП и заказа;
- сценарий проверяет перенос сумм, себестоимости, допрасхода, скидки, ответственного, источника и позиций, а также отсутствие дублей;
- rollback и независимая post-cleanup проверка ограничены уникальным синтетическим префиксом;
- source-contract проверен автоматически;
- migration и acceptance удалённо не запускались; runtime-результат и удалённая очистка не заявлены, production не использовался и не изменялся.
