...
- добавлена таблица `leader_user_invites` для приглашений сотрудников в CRM;
- добавлен trigger `leader_apply_profile_invite_trg`: новый профиль активируется только по действующему приглашению, без приглашения создаётся как неактивный;
- функция `leader_ensure_profile(text)` больше не активирует новых пользователей автоматически;
- существующие активные профили не изменялись;
- CRM v4 теперь показывает понятное состояние `Доступ ожидает активации`, если пользователь вошёл, но его профиль ещё не активирован;
- проверка Supabase после миграции: 4 профиля всего, 4 активных профиля, активные владельцы есть, приглашений пока нет.
...
11. Проверить, что существующий активный пользователь входит в CRM без изменений.
12. Тестовым новым пользователем без приглашения проверить состояние `Доступ ожидает активации`.
