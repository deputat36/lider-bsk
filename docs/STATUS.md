...
- отдельные проверки `Public lead audit copy check` и `Public lead audit helper check` защищают эти маркеры от случайного удаления.
...
8. Проверить цепочку через `leader_request_trace` или будущий CRM-виджет трассировки.
9. Повторно отправить ту же заявку с тем же `request_id` техническим тестом и убедиться, что audit показывает `duplicate`, а не `accepted`.
10. Пройти сценарий заявка → расчёт → КП → заказ → производство/монтаж.
