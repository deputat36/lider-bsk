...
- `crm/v4/index.html` подключает только базовые рабочие разделы текущего переноса.
...
1. Проверить в браузере перенесённый базовый контур CRM v4: вход, заявки, ручное создание заявки, карточку заявки, возврат к списку, потребности, контроль контактов, аудит заявок, диагностику.
