...
6. Открыть `https://deputat36.github.io/lider-bsk/crm/v4/?tab=user_admin` и проверить вкладку `Доступ` под owner/admin.
...
11. Повторно отправить ту же заявку с тем же `request_id` техническим тестом и убедиться, что audit показывает `duplicate`, а не `accepted`.
12. Пройти сценарий заявка → расчёт → КП → заказ → производство/монтаж.
