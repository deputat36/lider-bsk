...
- `index.html` подключает расширенные расчёты, КП и связанный заказ;
...
1. Проверить в браузере перенесённый контур CRM v4: вход, заявки, карточку заявки, потребности, расчёты, КП, связанный заказ, создание заказа из согласованного КП, контроль контактов, аудит заявок, диагностику.
2. Проверить аудит публичных заявок на реальной отправке формы.
3. Перенести отдельный раздел заказов и карточку заказа.
