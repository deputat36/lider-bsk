...
Общий обработчик сайта передаёт фактическое значение honeypot в Edge Function, а все публичные HTML-страницы используют cache-buster `public-lead-form.js?v=4`. Заполненный honeypot фиксируется как `suspicious` и не создаёт заявку.
