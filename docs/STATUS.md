...
- история и комментарии заявки: `lead-timeline.css`, `lead-timeline.js`, `lead-timeline-hooks.js`;
...
- историю и комментарии заявки;
- автоматическую запись событий при смене статуса и переносе следующего контакта;
...
- `ui-polish.css`, `responsive-ui-v2.css`, `followups.css`, `followups.js`, `lead-timeline.css`, `lead-timeline.js` и `lead-timeline-hooks.js` существуют и подключены в `index.html`;
...
1. Проверить в браузере перенесённый контур CRM v4: вход, меню, адаптивность, followups, дашборд, заявки, карточку заявки, таймлайн, потребности, расчёты, КП, создание заказа, связанный заказ, отдельный список заказов, карточку заказа, контроль заказов, финансы, производство, карточки производственных и монтажных заданий, контроль контактов, аудит заявок, диагностику.
