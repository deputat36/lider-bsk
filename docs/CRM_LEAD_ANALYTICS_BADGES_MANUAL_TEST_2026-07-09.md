...
- `crm/v4/assets/v4/lead-analytics-summary-v1.js` adds an aggregate summary block for normalized services and sources. Summary pills are clickable and apply search.
...
12. Click `Обновить заявки`.
13. Confirm that the badges and summary appear again after the list refreshes.
14. Switch away from the leads tab and back to `Заявки`.
15. Confirm that the badges still appear and are not duplicated multiple times on the same card.
16. Open a lead card and return to the list.
17. Confirm that the lead list still works and no browser console errors appear from `leads.js`, `lead-analytics-badges-v1.js` or `lead-analytics-summary-v1.js`.
...
- category clicked if summary click failed;
