...
2. Отправить заявку с пометкой `Тест CRM v4 audit v8`.
...
9. Если проводится техническая проверка дубля, повторная отправка с тем же `request_id` должна дать audit-событие `duplicate`, а не `accepted`.
10. Зафиксировать результат в `docs/CRM_V4_BROWSER_TEST_REPORT.md`.
