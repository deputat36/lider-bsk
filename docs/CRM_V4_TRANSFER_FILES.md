...
Заявки, история, потребности и контакты:
...
## Supabase-проверка для таймлайна
...
- `followups.css` и `followups.js` существуют и подключены в `index.html`;
- `lead-timeline.css`, `lead-timeline.js` и `lead-timeline-hooks.js` существуют и подключены в `index.html`;
...
5. Проверить, что `followups.css`, `followups.js`, `lead-timeline.css`, `lead-timeline.js`, `lead-timeline-hooks.js`, `ui-polish.css` и `responsive-ui-v2.css` не дают 404 в консоли браузера.
...
10. Проверить блок `Кому связаться сегодня` над статистикой заявок.
11. Если есть заявка с сегодняшним или просроченным следующим контактом, проверить `Открыть`, `+1 час`, `Завтра 10:00`.
12. Открыть карточку заявки.
13. Проверить блок `История и комментарии`.
14. Добавить тестовый комментарий в историю заявки.
15. Обновить историю и проверить, что комментарий остался.
16. Изменить статус безопасной тестовой заявки и проверить запись в истории.
17. Сохранить следующий контакт и проверить запись в истории.
18. Проверить возврат `Назад к списку`.
19. Добавить потребность.
20. Создать типовой или нестандартный расчёт.
21. Создать КП из сохранённого расчёта.
22. Открыть карточку КП.
23. Проверить копирование короткого и полного текста.
24. Открыть PDF строгий и PDF презентацию.
25. Отметить КП как согласованное.
26. Создать заказ из согласованного КП.
27. Проверить, что блок `Заказ` появился в карточке заявки.
28. Проверить, что повторное создание заказа заблокировано.
29. Открыть вкладку `Заказы`.
30. Проверить, что список заказов загрузился и не показывает рядом старые блоки заявок.
31. Открыть `Карточка заказа` из списка заказов.
32. Из карточки заказа открыть связанную заявку, если у заказа есть `lead_id`.
33. Открыть `Контроль заказов`.
34. Открыть `Финансы`.
35. Открыть `Производство`.
36. Проверить переключатель `Производство / Монтаж`.
37. Открыть карточку производственного задания, сохранить тестовое изменение и проверить печать листа.
38. Открыть карточку монтажного задания, сохранить тестовое изменение и проверить печать листа.
39. Открыть связанный заказ из производственного или монтажного задания.
40. Открыть `Контроль контактов`.
41. Открыть `Аудит заявок`.
42. Нажать `Проверить CRM` в диагностическом блоке.
43. Проверить консоль браузера: не должно быть 404 по CSS/JS из `assets/v4/`.
44. Проверить мобильную ширину: меню переносится без горизонтального скролла, карточки и кнопки не выходят за экран.
