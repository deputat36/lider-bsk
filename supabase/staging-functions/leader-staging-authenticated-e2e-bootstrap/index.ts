...
const COMMERCIAL_BRANCH_REF='refs/heads/agent/555-commercial-services-v1'
...
const TRUSTED_CONTEXTS=new Map<string,{eventName:string,workflowRefs:Set<string>}>([
  [COMMERCIAL_BRANCH_REF,{eventName:'push',workflowRefs:new Set([
    `${REPOSITORY}/.github/workflows/crm-staging-authenticated-e2e-dispatch.yml@${COMMERCIAL_BRANCH_REF}`,
    `${REPOSITORY}/.github/workflows/crm-staging-authenticated-e2e.yml@${COMMERCIAL_BRANCH_REF}`,
  ])}],
  ...
  [PRINT_PRIVACY_BRANCH_REF,{eventName:'push',workflowRefs:new Set([
    `${REPOSITORY}/.github/workflows/crm-staging-authenticated-e2e-dispatch.yml@${PRINT_PRIVACY_BRANCH_REF}`,
    `${REPOSITORY}/.github/workflows/crm-staging-authenticated-e2e.yml@${PRINT_PRIVACY_BRANCH_REF}`,
  ])}],
  [PRICING_BRANCH_REF,{eventName:'push',workflowRefs:PRICING_WORKFLOW_REFS}],
  [CATALOG_SOURCE_BRANCH_REF,{eventName:'push',workflowRefs:new Set([
    `${REPOSITORY}/.github/workflows/crm-staging-authenticated-e2e-dispatch.yml@${CATALOG_SOURCE_BRANCH_REF}`,
    `${REPOSITORY}/.github/workflows/crm-staging-authenticated-e2e.yml@${CATALOG_SOURCE_BRANCH_REF}`,
  ])}],
