...
import {
  ...
  INSTALLATION_CREATE_EDGE_CONTRACT_VERSION,
  MAX_BODY_BYTES,
  STAGING_PROJECT_REF,
  asObject,
  cleanText,
...
async function adminFetch(supabaseUrl: string, adminKey: string, path: string, init: RequestInit = {}) {
...
async function canonicalPermission(supabaseUrl: string, adminKey: string, actorId: string, permission: string) {
  const response = await adminFetch(supabaseUrl, adminKey, '/rest/v1/rpc/leader_actor_has_crm_action_rpc', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ p_actor_id: actorId, p_action: permission }),
  })
  if (!response.ok) return { error: json(500, { error: 'permission_check_failed' }) }
...
function safeRpcError(value: unknown) {
  ...
  const known = new Set(['validation_error','unknown_action','forbidden','not_found','conflict','duplicate_request','persistence_failed'])
...
Deno.serve(async (req: Request) => {
  ...
  const publicKey = preferredEnvironmentKey(Deno.env.get('SUPABASE_ANON_KEY'), Deno.env.get('SUPABASE_PUBLISHABLE_KEYS'))
  const adminKey = preferredEnvironmentKey(Deno.env.get('SUPABASE_SERVICE_ROLE_KEY'), Deno.env.get('SUPABASE_SECRET_KEYS'))
  if (!supabaseUrl || !publicKey || !adminKey) return json(500, { error: 'server_not_configured' })

  if (projectRefFromUrl(supabaseUrl) !== STAGING_PROJECT_REF) {
    return json(503, { error: 'wrong_environment', expected: 'staging', contract: INSTALLATION_CREATE_EDGE_CONTRACT_VERSION })
  }

  const contentLength = Number(req.headers.get('content-length') || 0)
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY_BYTES) return json(413, { error: 'payload_too_large' })
  ...
  const validation = validateInstallationCreateRequest(input)
  if (!validation.ok) {
  ...
  for (const permission of validation.permissions) {
    const result = await canonicalPermission(supabaseUrl, adminKey, checked.user.id, permission)
    if (result.error) return result.error
    if (!result.allowed) {
      return json(403, {
        error: 'forbidden', action: INSTALLATION_CREATE_ACTION, permission,
        contract: INSTALLATION_CREATE_EDGE_CONTRACT_VERSION,
      })
    }
  }

  const rpcResponse = await adminFetch(supabaseUrl, adminKey, '/rest/v1/rpc/leader_create_installation_job_from_order_rpc', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      p_payload: {
  ...
  if (!rpcResponse.ok) {
    return json(500, {
      ...
      error: { code: 'persistence_failed', message: 'Installation create RPC unavailable' },
    })
  }

  const result = asObject(await rpcResponse.json())
  if (!result) return json(500, { error: 'invalid_rpc_response' })
  if (result.ok !== true) {
