...
import {
  ...
  PRODUCTION_CREATE_EDGE_CONTRACT_VERSION,
  STAGING_PROJECT_REF,
  asObject,
  cleanText,
  isJwtApiKey,
...
async function canonicalPermission(
  ...
  if (!response.ok) {
    console.error('leader-crm-production-create permission transport failure', {
...
Deno.serve(async (req: Request) => {
  ...
  if (projectRefFromUrl(supabaseUrl) !== STAGING_PROJECT_REF) {
    return json(503, {
      ...
      expected: 'staging',
      contract: PRODUCTION_CREATE_EDGE_CONTRACT_VERSION,
    })
  }
  ...
  const validation = validateProductionCreateRequest(input)
  if (!validation.ok) {
  ...
  if (!rpcResponse.ok) {
    console.error('leader-crm-production-create rpc transport failure', {
    ...
    return json(500, {
      ok: false,
      request_id: validation.request.request_id,
      error: { code: 'persistence_failed', message: 'Production create RPC unavailable' },
    })
  }
