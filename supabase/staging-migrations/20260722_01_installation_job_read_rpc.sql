-- STAGING ONLY.
-- Reproducible source for 20260722050355 / staging_installation_job_read_rpc_20260722.
-- Safe read projection for installation job card. Never apply to production.
...
begin
  ...
  if to_regclass('public.leader_installation_jobs') is null
     ...
     or to_regclass('public.leader_installation_comments') is null
     or to_regclass('public.leader_orders') is null
     or to_regclass('public.leader_production_jobs') is null then
    raise exception 'installation_read_requires_staging_schema';
  end if;
end
$guard$;
