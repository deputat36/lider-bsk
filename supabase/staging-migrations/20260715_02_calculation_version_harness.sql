...
-- Depends on:
--   20260713_00_environment_guard.sql
--   20260713_01_design_task_harness.sql (profiles, leads, needs and private receipts)
-- Source-only candidate. Do not apply without explicit approval.
...
begin
  ...
  if to_regclass('leader_private.leader_command_receipts') is null then
    raise exception 'calculation_version_requires_command_receipts';
  end if;

  if to_regclass('public.leader_user_profiles') is null
...
comment on table public.leader_lead_calculation_items is
  'STAGING harness: item snapshot belonging to one immutable calculation version.';
...
begin
  ...
  if v_actor_id is null
     ...
     or v_source_calculation_id is null
     or v_idempotency_key is null
     or char_length(v_idempotency_key) not between 1 and 160 then
    return jsonb_build_object(
      'ok', false,
      'request_id', v_request_id,
      'error', jsonb_build_object('code', 'invalid_payload', 'message', 'Required command fields are missing')
    );
  end if;
  ...
  if v_items is null or jsonb_typeof(v_items) <> 'array' then
    return jsonb_build_object(
      'ok', false,
      'request_id', v_request_id,
      'error', jsonb_build_object('code', 'empty_items', 'message', 'items must be an array')
    );
  end if;
  ...
  if v_item_count < 1 then
    return jsonb_build_object(
      'ok', false,
      'request_id', v_request_id,
      'error', jsonb_build_object('code', 'empty_items', 'message', 'items must not be empty')
    );
  end if;
  ...
  if v_source.updated_at <> v_expected_updated_at then
    return jsonb_build_object(
      'ok', false,
      'request_id', v_request_id,
  ...
  loop
    ...
    if v_name is null
       ...
       or v_qty <= 0
       or v_qty > 1000000
       or v_contractor_price < 0
       or v_contractor_price > 1000000000
       or v_client_price < 0
       or v_client_price > 1000000000
       or v_sort_order < 0
       or v_sort_order > 1000000
    ...
    v_item_markup := case
      when v_contractor_price > 0 then round(((v_client_price - v_contractor_price) / v_contractor_price) * 100, 2)
      else 0
    end;
    v_item_margin := case
      when v_client_sum > 0 then round((v_item_profit / v_client_sum) * 100, 2)
      else 0
    end;
  ...
  v_margin_total := case
    when v_client_total > 0 then round((v_profit_total / v_client_total) * 100, 2)
    else 0
  end;
  ...
  if v_client_total <= 0 or v_profit_total < 0 then
    return jsonb_build_object(
      'ok', false,
      'request_id', v_request_id,
      'error', jsonb_build_object('code', 'invalid_totals', 'message', 'Client total must be positive and profit must not be negative')
    );
  end if;
  ...
  v_title := coalesce(nullif(btrim(v_payload ->> 'title'), ''), v_source.title || ' — версия ' || v_next_version::text);
  v_public_comment := coalesce(v_payload ->> 'public_comment', v_source.public_comment);
  v_internal_comment := coalesce(v_payload ->> 'internal_comment', v_source.internal_comment);
  ...
  loop
    ...
    insert into public.leader_lead_calculation_items (
      ...
      v_new_calculation.id,
      v_new_calculation.lead_id,
      v_catalog_id,
      v_category,
      v_item_type,
      v_name,
  ...
  from public.leader_lead_calculation_items as item_row
  where item_row.calculation_id = v_new_calculation.id;

  v_response := jsonb_build_object(
    'ok', true,
    'request_id', v_request_id,
    'source_calculation_id', v_source.id,
    'calculation', to_jsonb(v_new_calculation),
...
exception
  ...
  when others then
    ...
    raise log 'leader_create_calculation_version_rpc failed request_id=% sqlstate=%', v_request_id, v_exception_state;
    return jsonb_build_object(
      'ok', false,
      'request_id', v_request_id,
      'error', jsonb_build_object('code', 'calculation_version_create_failed', 'message', 'Calculation version could not be persisted')
    );
end
$function$;
...
comment on function public.leader_create_calculation_version_rpc(jsonb) is
  'STAGING ONLY candidate: atomically creates an immutable calculation version, item snapshot and idempotency receipt.';
