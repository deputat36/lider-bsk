...
do $preflight$
begin
  if to_regprocedure('public.leader_create_initial_calculation_rpc(jsonb)') is null or md5(pg_get_functiondef(to_regprocedure('public.leader_create_initial_calculation_rpc(jsonb)'))) <> 'ba8ad5920edc71f1f6a9e4316dc3bd37' then raise exception 'calculation_forward_preflight_failed'; end if;
  if to_regprocedure('leader_private.leader_create_calculation_version_rpc_internal_v1(jsonb)') is null or md5(pg_get_functiondef(to_regprocedure('leader_private.leader_create_calculation_version_rpc_internal_v1(jsonb)'))) <> '7a68e970d4e0f528a46c83fc36d5b130' then raise exception 'calculation_forward_preflight_failed'; end if;
end $preflight$;
...
declare
  ...
  v_cost_sum numeric := 0; v_client_sum numeric := 0; v_profit numeric; v_margin numeric; v_warning text := 'ok'; v_warnings jsonb := '[]'::jsonb;
...
begin
  ...
  for v_item in select value from jsonb_array_elements(v_payload->'items') loop
    ...
    if btrim(coalesce(v_item->>'name',''))='' or v_qty is null or v_qty<=0 or v_qty>1000000 or v_cost is null or v_cost<0 or v_cost>1000000000 or v_price is null or v_price<0 or v_price>1000000000 then
      return jsonb_build_object('ok',false,'request_id',v_request_id,'error',jsonb_build_object('code','validation_error'));
    end if;
  ...
  if v_client_sum=0 then v_warnings := v_warnings || jsonb_build_array('Бесплатная работа'); end if;
  if v_profit<0 then v_warnings := v_warnings || jsonb_build_array('Расчёт убыточный');
  elsif v_profit=0 then v_warnings := v_warnings || jsonb_build_array('Работа в ноль');
  elsif v_margin<15 then v_warnings := v_warnings || jsonb_build_array('Маржа ниже 15%'); end if;
  if jsonb_array_length(v_warnings)>0 then v_warning := 'warning'; end if;
  ...
  insert into public.leader_lead_calculations(lead_id,need_id,client_id,title,status,version_number,client_total,contractor_cost,profit,margin_percent,warning_level,warnings,public_comment,internal_comment,created_by,updated_by,is_current_revision,created_at,updated_at)
    values(v_lead_id,v_need_id,v_need.client_id,v_title,'Черновик',v_count,v_client_sum,v_cost_sum,v_profit,v_margin,v_warning,v_warnings,v_public,v_internal,v_actor,v_actor,true,v_now,v_now) returning * into v_calc;
  for v_item in select value from jsonb_array_elements(v_payload->'items') loop
...
begin
  ...
  if v_client_total = 0 then
    v_warnings := v_warnings || jsonb_build_array('Бесплатная работа');
  end if;
  if v_profit_total < 0 then
    v_warnings := v_warnings || jsonb_build_array('Расчёт убыточный');
  elsif v_profit_total = 0 then
    v_warnings := v_warnings || jsonb_build_array('Работа в ноль');
  elsif v_margin_total < 15 then
    v_warnings := v_warnings || jsonb_build_array('Маржа ниже 15%');
  end if;
  if jsonb_array_length(v_warnings) > 0 then v_warning_level := 'warning'; end if;
