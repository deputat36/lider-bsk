-- STAGING ONLY.
-- Applied as 20260722194950 / staging_installation_read_capabilities_20260722.
-- Adds server-projected read/write capabilities for the isolated staging UI.
...
begin
  ...
  if to_regprocedure('public.leader_read_installation_job_rpc(uuid,uuid)') is null then
    raise exception 'installation_read_rpc_missing';
  end if;
end
$guard$;
