...
async function rest(supabaseUrl: string, apiKey: string, bearer: string, path: string, init: RequestInit = {}) {
...
async function serviceRest(supabaseUrl: string, serviceRole: string, path: string, init: RequestInit = {}) {
  return rest(supabaseUrl, serviceRole, serviceRole, path, init)
}

async function readOne(supabaseUrl: string, serviceRole: string, path: string) {
  const res = await serviceRest(supabaseUrl, serviceRole, path)
...
async function getUserFromRequest(req: Request, supabaseUrl: string, anonKey: string) {
  ...
  const userRes = await rest(supabaseUrl, anonKey, token, '/auth/v1/user')
...
async function checkUser(req: Request, supabaseUrl: string, anonKey: string, serviceRole: string) {
  ...
  if (!res.ok) return { error: json(403, { error: 'profile_check_failed' }) }
...
async function ensureProfile(req: Request, supabaseUrl: string, anonKey: string) {
  ...
  const email = cleanText(String(userCheck.user.email || '').toLowerCase(), 240)
  if (!email) return json(400, { error: 'authenticated_email_required' })
  const res = await rest(supabaseUrl, anonKey, userCheck.token, '/rest/v1/rpc/leader_ensure_profile', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ user_email: email }),
  })
  if (!res.ok) return json(500, { error: 'profile_rpc_failed', details: await res.text() })
  return json(200, { ok: true, profile: await res.json() })
}
...
async function dashboard(supabaseUrl: string, serviceRole: string) {
  const statsRes = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_leads?select=id,status&status=not.eq.Спам&order=created_at.desc&limit=300')
  if (!statsRes.ok) return json(500, { error: 'dashboard_stats_failed', details: await statsRes.text() })
  const all = await statsRes.json()
  const recentRes = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_leads?select=' + encodeURIComponent(leadFields) + '&status=not.eq.Спам&order=created_at.desc&limit=12')
...
async function listLeads(supabaseUrl: string, serviceRole: string, body: Record<string, unknown>) {
  ...
  const res = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_leads?select=' + encodeURIComponent(leadFields) + '&order=created_at.desc&limit=' + limit)
  if (!res.ok) return json(500, { error: 'leads_read_failed', details: await res.text() })
  return json(200, { ok: true, leads: await res.json() })
}

async function listOrders(supabaseUrl: string, serviceRole: string) {
  const res = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_orders?select=' + encodeURIComponent(orderFields) + '&order=created_at.desc&limit=80')
...
async function createLead(supabaseUrl: string, serviceRole: string, body: Record<string, unknown>) {
  ...
  const res = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_leads?select=' + encodeURIComponent(leadFields), { method: 'POST', headers: { 'Content-Type': 'application/json', 'Prefer': 'return=representation' }, body: JSON.stringify(payload) })
...
async function updateLead(supabaseUrl: string, serviceRole: string, body: Record<string, unknown>) {
  ...
  const res = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_leads?id=eq.' + encodeURIComponent(id) + '&select=' + encodeURIComponent(leadFields), { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Prefer': 'return=representation' }, body: JSON.stringify(patch) })
...
async function ensureClient(supabaseUrl: string, serviceRole: string, ownerId: string, body: Record<string, unknown>) {
  ...
  if (phone) {
    const foundRes = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_clients?phone=eq.' + encodeURIComponent(phone) + '&select=' + encodeURIComponent(clientFields) + '&limit=1')
    if (foundRes.ok) { const found = await foundRes.json(); if (Array.isArray(found) && found[0]) return { ok: true, client: found[0], existed: true } }
  }
  const insertRes = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_clients?select=' + encodeURIComponent(clientFields), { method: 'POST', headers: { 'Content-Type': 'application/json', 'Prefer': 'return=representation' }, body: JSON.stringify({ owner_id: ownerId, name, phone, source, comment }) })
...
async function createOrder(supabaseUrl: string, serviceRole: string, ownerId: string, body: Record<string, unknown>) {
  ...
  const orderRes = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_orders?select=' + encodeURIComponent(orderFields), { method: 'POST', headers: { 'Content-Type': 'application/json', 'Prefer': 'return=representation' }, body: JSON.stringify(orderPayload) })
  ...
  if (order?.id) {
    ...
    if (itemRows.length) {
      const itemRes = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_order_items', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Prefer': 'return=minimal' }, body: JSON.stringify(itemRows) })
    ...
    if (leadId) await serviceRest(supabaseUrl, serviceRole, '/rest/v1/leader_leads?id=eq.' + encodeURIComponent(leadId), { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Prefer': 'return=minimal' }, body: JSON.stringify({ status: 'Создан заказ', converted_order_id: order.id, converted_client_id: clientResult.client?.id || null, converted_at: new Date().toISOString() }) })
  }
...
async function createOrderFromOffer(supabaseUrl: string, serviceRole: string, ownerId: string, actorEmail: string, body: Record<string, unknown>) {
  ...
  const rpcRes = await serviceRest(supabaseUrl, serviceRole, '/rest/v1/rpc/leader_create_order_from_offer_rpc', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ p_payload: { ...body, offer_id: offerId, actor_id: ownerId, actor_email: actorEmail || null } }),
  })
...
Deno.serve(async (req: Request) => {
  ...
  if (action === 'ensure_profile') return await ensureProfile(req, supabaseUrl, anonKey)
